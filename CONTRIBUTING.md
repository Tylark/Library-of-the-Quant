# Contributing

Library of the Quant is an educational PDF library. Contributions should preserve clarity, consistency, and the educational-only purpose of the project.

## Rules

- Do not make trading recommendations.
- Do not present examples as investment, trading, or risk-management advice.
- Treat PDFs as finished artifacts.
- Store final PDFs in `manuals/pdf/`.
- Store companion Markdown notes in `manuals/markdown/`.
- Use consistent naming.
- Assign QRK IDs sequentially when adding manuals.
- Update relevant indexes whenever manuals are added.
- Mark uncertain or unverified metadata as `Needs review`.

## Naming

Use descriptive lowercase filenames with hyphens where practical. Keep the QRK ID visible once assigned.

Example:

```text
qrk-0001-probability-foundations-field-guide.pdf
qrk-0001-probability-foundations-field-guide.md
```
